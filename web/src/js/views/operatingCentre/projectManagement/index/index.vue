            ...
            >
              <template slot-scope="scope">
                <router-link
                  :to="{
                    path:
                      '/operatingCentre/projectManagement/detail/' +
                      scope.row.id
                  }"
                  class="link"
    ...
    <el-dialog :visible.sync="updateSpecification" title="编辑项目" width="40%">
      ...
      >
        ...
        >
          <el-button
            type="primary"
            @click="updateSubmitForm('updateProjectFrom')"
          >保存</el-button
          >
...
export default {
  ...
  methods: {
    ...
    searchName() {
      ...
      filterGetProjects(this.user.uid, this.filter).then(res => {
        console.log(res);
    ...
    async fetchData() {
      ...
      // 组织机构下拉框
      const r = await requestParams(getOrganizations, this.page);
      const organList = r.content.content;
    ...
    searcClickOrgan(data) {
      ...
      console.log(organlist);
      this.filter.id = organlist[0].value == null && organlist[0].value == undefined ? '' : organlist[0].value;
    },
    ...
    update(data) {
      console.log(data);
      this.updateProjectFrom = data;
      this.updateSpecification = true;
    },
    // 修改项目
    updateProject() {
      console.log(this.updateProjectFrom);
