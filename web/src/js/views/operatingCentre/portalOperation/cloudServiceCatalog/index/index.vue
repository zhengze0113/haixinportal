        ...
        <el-col>
          <div>
            ...
            <div slot="footer" class="dialog-footer" style="float:right;margin-top:20px;">
              ...
              <el-button size="mini" type="primary" @click="createdcloudServiceCatalogs();close();" >确认</el-button>
            </div>
          </div>
        </el-col>
      </el-row>
  ...
  methods: {
    async fetchData() {
      ...
      this.list = res;
       console.log(this.list);
        const re = await requestParams(Servicecatalog,);
    ...
    loadNode(node, resolve) {
      ...
      if (node.level === 0) {
       ...
       Servicecatalog(node.data.id).then(res => {
          return resolve(res);
          console.log(res)
        });
      }
   ...
   close() {
      this.contents = {};
      this.$nextTick(() => {
        this.$refs[this.contents].resetFields();
      });
    },
    // 获取选择父节点id，name
    confirmChoice(data) {
