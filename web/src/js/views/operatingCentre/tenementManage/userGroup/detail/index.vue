<template>
  <div class="app-container">
    <close-card :show-arrow="true" title="用户组摘要" icon="list">
      <div>
        <el-row :gutter="2">
    ...
    <close-card :show-arrow="true" title="用户组详情列表" icon="list">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //批量解除用户
      batchDeleted(){
        if(this.batchDeletedObject == null || this.batchDeletedObject.length == 0) {this.$notify.error('请选择一个或多个用户进行解除'); return;}
        else{
      ...
      //添加机构和用户的关系
      addRelation(){
        if(this.organizationObject == null){this.$notify.error('请选择要建立关系的机构'); return;}
        else{
