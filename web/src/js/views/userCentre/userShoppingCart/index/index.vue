  ...
  <div>
    <el-col class="box-card" style="margin-top: 10px;">
      <el-row :gutter="20" class="population">
        ...
        >
          <el-col :span="24" class="title">
            <span style="line-height: 28px;">数据统计</span>
            <div class="right">
              <el-tabs
                v-model="activeDate"
                :offset="20"
...
<script>
import { getUserStatistics, getStatisticsTotle, getStatisticsMoney } from '@/api/ordersCenter';
import {
  getUserInfo
} from '@/utils/auth';
  ...
  methods: {
    async getList() {
      ...
      if (this.search1.startTime != undefined) {
        await getUserStatistics(this.data.uid, this.search1).then(res => {
          ...
          this.search1.userId = this.data.uid;
        });
        ...
        const res = await requestParams(getUserStatistics, this.data.uid);
        for (let i = 0; i < res.content.length; i++) {
          const arr = { '日期': null, '订单量': null };
          arr['日期'] = res.content[i].date;
          arr['订单量'] = res.content[i].value;
      ...
      this.search.userId = this.data.uid;
