<template>
  <div>
        ...
        <div style="overflow: hidden;background:#fff;border-radius: 4px;">
          ...
          >
            ...
            <el-table-column label="操作" align="center">
              <template slot-scope="scope">
                <el-dropdown v-if="scope.row.stage==0">
                  ...
                  <el-dropdown-menu slot="dropdown" >
                    <el-dropdown-item v-if="scope.row.stage==0" @click.native="stage(scope.row.id)">
                      通过
                    </el-dropdown-item>
                    <el-dropdown-item v-if="scope.row.stage==0" @click.native="noPass(scope.row.id)">
                      不通过
                    </el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
              ...
              <!-- <el-link v-if="scope.row.stage==0" type="text" class="link" @click="urge(scope.row.id)">催办</el-link> -->
              </template>
            </el-table-column>
          </el-table>
    ...
    <el-dialog :visible.sync="dialogFormVisible" title="操作提示" width="30%">
      <div class="warn">
        <el-alert
          :closable="false"
          show-icon
          type="warning"
        >
          <span slot="title" class="warnTitle">确认【不通过】操作吗？</span>
        </el-alert>
        <el-row style="padding:2px 10px">注：操作后将无法修改</el-row>
      </div>
      <el-input
        :rows="2"
        v-model="reason"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogFormVisible = false;stageDown()">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    jump(row) {
      this.$router.push({
        path:
          '/tenementCentre/flowApproval/detail/example/' + row.orderId,
        query: { stage: row.stageName }
      });
    },
    stage(id) {
      this.$confirm('确定通过该流程吗?', '通过操作', {
    ...
    stageDown() {
      const stage = { reason: this.reason };
      this.reason = '';
      getStage(this.noPassId, 2, stage).then(r => {
        if (r.code == 200 || r.code == 201) {
    ...
    getList() {
      var tenantId = { tenantId: this.userInfo.tenant };
      getOrdersStatistics(tenantId).then(r => {
        this.statistics = r.content;
      });
      this.search.tenantId = this.userInfo.tenant;
